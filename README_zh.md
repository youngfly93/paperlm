...
部署服务前可以用 `paperlm-tools warmup` 预下载并初始化可选引擎，
避免第一份真实 PDF 请求承担模型下载和冷启动成本。warmup 在受控
worker subprocess 里运行，超时或超内存只会杀掉 warmup worker，
不会拖死父进程。
...
命令会校验是否真的跑到了目标引擎；例如 `--engine docling` 必须返回
`engine_used=docling`，否则会失败并提示检查 extra 或运行环境。
